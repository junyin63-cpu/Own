...
/**
 * 解压 zTXt / iTXt 中的 deflate 压缩数据
 */
async function decompressPngText(data) {
  try {
    const stream = new Response(data).body.pipeThrough(new DecompressionStream('deflate'));
    const result = await new Response(stream).arrayBuffer();
    return new TextDecoder('utf-8').decode(result);
  } catch (e) {
    throw new Error("解压角色数据失败：" + e.message);
  }
}
    ...
    reader.onload = async (e) => {
      try {
        ...
        while (offset < view.byteLength) {
          ...
          // ========== zTXt (压缩文本) ==========
            ...
            if (keyword === 'chara') {
              ...
              try {
                const decompressed = await decompressPngText(compressedData);
                const base64Data = decompressed;
                const decodedString = atob(base64Data);
                const bytes = new Uint8Array(decodedString.length);
                for (let i = 0; i < decodedString.length; i++) {
                  bytes[i] = decodedString.charCodeAt(i);
                ...
                return reject(new Error(`解析压缩的角色数据失败: ${e.message}`));
              }
            }
          }
            ...
            if (keyword === 'chara') {
              ...
              try {
                ...
                if (compressionFlag === 1) {
                  finalText = await decompressPngText(textBytes);
                } else {
                  finalText = new TextDecoder('utf-8').decode(textBytes);
                }
                
                const base64Data = finalText;
                const decodedString = atob(base64Data);
                const bytes = new Uint8Array(decodedString.length);
                for (let i = 0; i < decodedString.length; i++) {
                  bytes[i] = decodedString.charCodeAt(i);
                ...
                return reject(new Error(`解析国际化角色数据失败: ${e.message}`));
              }
            }
          }
        ...
        if (charaData) {
          ...
          reject(new Error('在PNG中未找到有效的角色数据 (tEXt/zTXt/iTXt chunk not found)。'));
        }
      } catch (error) {
